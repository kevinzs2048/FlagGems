...
def _pack_lhs_qai8dxp_bf16_mr4_kernel(
    ...
    """Pack one symmetric (possibly padded) MR4 panel into ``qai8dxp``."""
    panel = tl.program_id(0)
    rows = panel * 4 + tl.arange(0, 4)
    source_rows = tl.minimum(rows, M - 1)
    lanes8 = tl.arange(0, 8)
    absmax_lanes = tl.zeros((4, 8), tl.float32)
    for off in tl.range(0, K, 8, loop_unroll_factor=1):
        values = tl.load(
            x_ptr
        ...
        absmax_lanes = tl.maximum(absmax_lanes, tl.abs(values))
    absmax = tl.maximum(tl.max(absmax_lanes, axis=1), 1.0e-8)
    scales = absmax / 127.0
    ...
    for off in tl.range(0, K, 8, loop_unroll_factor=1):
        ...
        rounded = libdevice.rint(values * inv_scales[:, None]).to(tl.int32)
        quantized = tl.minimum(tl.maximum(rounded, -127), 127).to(tl.int8)
        tl.store(
...
def _w8_qai8dxp_prefill_short_tail_kernel(
    ...
    """Legacy asymmetric KAI comparator for M4/M8 prefill."""
...
def _w8_qai8dxp_prefill_m12_kernel(
    ...
    """Legacy asymmetric KAI comparator for an M8 plus M4 K loop."""
    pid_n = tl.program_id(1)
    cols = tl.arange(0, 4)
    panel_offsets = tl.arange(0, 128)
    meta_lanes = tl.arange(0, 4)
